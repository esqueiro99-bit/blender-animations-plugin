...
function RigManager:setRig(rigModel: Types.RigModelType?): any
	...
	-- Scan decals on the active rig model (Head only)
	self:scanRigDecals(rigModel)
...
function RigManager:cleanup()
	...
	self:clearWarnings()
	self.boneWeights = {}
	self:scanRigDecals(nil)
end

function RigManager:scanRigDecals(rigModel: Model?)
	if not rigModel then
		State.activeDecals:set({})
		State.decalNames:set({})
		return
	end
	...
	local head = rigModel:FindFirstChild("Head") or rigModel:FindFirstChild("head") or rigModel:FindFirstChild("Face")
	if not head then
		for _, desc in ipairs(rigModel:GetDescendants()) do
			if desc:IsA("BasePart") and (string.lower(desc.Name) == "head" or string.lower(desc.Name) == "face") then
				head = desc
				break
			end
		end
	...
	local targetContainer = head or rigModel
	for _, desc in ipairs(targetContainer:GetDescendants()) do
		if desc:IsA("Decal") then
			if not decals[desc.Name] then
				decals[desc.Name] = desc
	...
	State.activeDecals:set(decals)
	State.decalNames:set(names)
end

function RigManager:setDecalTransparency(decalName: string, transparency: number)
	local decals = State.activeDecals:get()
	local decal = decals[decalName]
	if decal then
		decal.Transparency = math.clamp(transparency, 0, 1)
	end
end

function RigManager:soloDecal(targetDecalName: string)
	local decals = State.activeDecals:get()
	for name, decal in pairs(decals) do
		decal.Transparency = (name == targetDecalName) and 0 or 1
	end
end

function RigManager:toggleDecal(targetDecalName: string)
	local decals = State.activeDecals:get()
	local decal = decals[targetDecalName]
	if decal then
		decal.Transparency = (decal.Transparency < 0.5) and 1 or 0
	end
end

function RigManager:setAllDecals(visible: boolean)
	local decals = State.activeDecals:get()
