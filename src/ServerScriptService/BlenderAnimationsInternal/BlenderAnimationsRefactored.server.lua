...
local internal = script:FindFirstChild("BlenderAnimationsInternal") 
...
local Components = internal.Components
local Packages = internal.Packages
...
do -- Creates the plugin
	...
	local function safeCreateTab(tabName: string, tabModule: any)
		local ok, content = pcall(function()
			return tabModule.create(services)
		end)
		if ok and content then
			return content
		else
			warn("[BlenderAnimations] Erro ao carregar aba " .. tabName .. ": " .. tostring(content))
			return {
				New("TextLabel")({
					Text = "Erro ao carregar aba " .. tabName .. ": " .. tostring(content),
	...
	local tabContent = {
		Player = safeCreateTab("Player", PlayerTab),
		Rigging = safeCreateTab("Rigging", RiggingTab),
		Tools = safeCreateTab("Tools", ToolsTab),
		More = safeCreateTab("More", MoreTab),
	}
	...
	print("[DEBUG-DECAL] mainWidget inicializado. Estado inicial Enabled = " .. tostring(mainWidget.Enabled))

	table.insert(State.connections, mainWidget:GetPropertyChangedSignal("Enabled"):Connect(function()
		print("[DEBUG-DECAL] mainWidget.Enabled mudou para: " .. tostring(mainWidget.Enabled))
		handleMainWidgetEnabledChanged(mainWidget.Enabled)
		enableButton:SetActive(mainWidget.Enabled)
	end))
	mainWidget:BindToClose(function()
		print("[DEBUG-DECAL] mainWidget fechado pelo usuario.")
	...
	table.insert(State.connections, helpWidget:GetPropertyChangedSignal("Enabled"):Connect(function()
		...
		helpButton:SetActive(helpWidget.Enabled)
	end))
	...
	table.insert(State.connections, helpButton.Click:Connect(function()
		local nextState = not helpWidget.Enabled
		helpWidget.Enabled = nextState
		handleHelpWidgetEnabledChanged(nextState)
		helpButton:SetActive(nextState)
	end))
end
...
print("[BlenderAnimations Decals] Plugin inicializado com sucesso! Pronto para uso.")
