...
local DecalControls = require(script.Parent.Parent.Components.DecalControls)
...
function ToolsTab.create(services: any)
	...
	if cameraControls then
		...
		table.insert(components, createToolDivider(2))
	end

	local decalControls = DecalControls.createDecalControlsUI(services, 3)
	if decalControls then
		table.insert(components, decalControls)
		table.insert(components, createToolDivider(4))
	end

	local keyframeNaming = KeyframeNaming.createKeyframeNamingUI(services, 5)
	if keyframeNaming then
		table.insert(components, keyframeNaming)
		table.insert(components, createToolDivider(6))
	end
